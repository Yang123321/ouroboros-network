...
runThreadNetwork ThreadNetworkArgs
  ...
  where
    ...
    forkVertex
      ...
      where
        ...
        loopBody mbS = do
          ...
          withRegistry $ \nodeRegistry -> do
            nodeTestBtime <- cloneTestBlockchainTime sharedTestBtime nodeRegistry
            ...
            -- stop threads that depend on/stimulate the ChainDB
            NodeKernel.stopBGThreads kernel

            -- close the ChainDB
            let chainDB = getChainDB kernel
            ch <- ChainDB.toChain chainDB

            -- stop the node's internal threads (CDB background threads, the
            -- 'blockFetchLogic' thread, etc)
            releaseAll nodeRegistry

            ChainDB.closeDB chainDB

            atomically $ writeTVar vertexStatusVar $ VDown ch
    ...
    forkEbbProducer btime registry nextEbbSlotVar cfg chainDB epochInfo =
        fmap cancelThread $ forkLinkedThread registry $ go 0
      where
        go :: EpochNo -> m ()
        go !epoch = do
    ...
    forkNode epochInfo varRNG btime registry coreNodeId nodeInfo = do
      ...
      let stop' = do hTx; hEBB; NodeKernel.stopBGThreads nodeKernel
      return (nodeKernel{NodeKernel.stopBGThreads = stop'}, LimitedApp app)
